...
| TD-7 | ~~`authService.ts:370`~~ **2026-06-23 闭环** | SMS 验证码改用 `crypto.randomInt(100000, 1000000)`（D-023）。**注：真实 SMS 供应商（Twilio/阿里云）仍待接入，仅生成侧已修** | 短信验证码生成密码学安全；发送侧待供应商 |
...
| TD-7  | ~~SMS 验证码用 `Math.random()` 生成~~ **2026-06-23 闭环（生成侧）：改用 `crypto.randomInt(100000, 1000000)`（D-023）。真实 SMS 供应商接入仍待办，作为独立运营任务追踪，不挂技术债**                                                                                                                                                | 已闭环（生成侧）          | §9.2 审计 → 2026-06-23        |
...
| TD-19 | Repository 范式三选一分叉并存：(a) 裸 Pool + `runWithTenant`（`ImMessageRepository` + D-023 新增的 `User/OAuth/Sms` repo）、(b) drizzle client（`ConversationRepository`）、(c) drizzle 无 null 降级（`AuditLogRepository`）。D-023 已把 auth 这一处对齐到 (a)，其余两处（Conversation / AuditLog）统一留待后续治理——触动 drizzle mock 层与对应单测，代价数倍于本次 | 🟡 中（已显式记账）       | D-023 → 2026-06-23            |

---

## 10. 最近更新时间

- 2026-06-23（**架构评估后续落地**（D-023）：4 项——① IM `deleteMessage`/`markAsRead` 改单行查询（`findById`），消除"最近 200 条"误报 not found；② Outbox 定时清理任务 + `OUTBOX_RETENTION_DAYS`/`OUTBOX_CLEANUP_INTERVAL_MS` 配置（`retentionDays=0` 可禁用）；③ `refreshTokens` 加 `is_active` 守卫，堵禁用账户靠旧 refresh token 续期 7 天的漏洞；④ `authService` 裸 SQL 下沉到新增 `UserRepository`/`OAuthAccountRepository`/`SmsCodeRepository`（裸 Pool 范式，对齐 `ImMessageRepository`），测试转方法级 mock + 3 个 repo 单测；TD-7 SMS `crypto.randomInt` 收尾；新增 TD-19 记 repository 范式分叉。build + lint + layers + naming + 581 后端单测全绿；outbox cleanup / auth repo 真库行为待 integration 验证）
