...
describe.skipIf(!databaseUrl)('Multi-Tenant Isolation (RLS)', () => {
  ...
  async function seedTestData(): Promise<void> {
    ...
    try {
      ...
      await client.query(`INSERT INTO conversations (tenant_id, id) VALUES ($1, $2) ON CONFLICT DO NOTHING`, [
        TENANT_A,
        CONVERSATION_A,
      ]);
      await client.query(
  ...
  describe('Edge Cases and Attack Vectors', () => {
    ...
    it('should enforce RLS even with FORCE ROW LEVEL SECURITY', async () => {
      ...
      try {
        ...
        await expect(client.query(`SELECT id FROM conversations WHERE tenant_id = $1`, [TENANT_A])).rejects.toThrow(
          'Tenant context',
        );
