-- Backlog plugin: owns its tables, indexes and RLS policies.
-- Idempotent so it coexists with the legacy core migration (0017) on already-provisioned databases
-- and provisions the tables from scratch on fresh ones.
...
-- Row-Level Security: every row is scoped to app.current_tenant set per connection by the plugin.
